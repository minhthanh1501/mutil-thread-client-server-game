...
public class HandleThread extends Thread{
	...
	public String whoDeclareWinner;
	...
	public static int babich = 0;
	public boolean whoHaveBabich = false;
	...
	public synchronized void doRead() {
		try {
			...
			case "registrationAccountName":
				...
				if (handleThreadList.size() == 4) {
					...
					for (int i = 0; i < handleThreadList.size(); i++) {
						...
						if (i == 0 && whoHaveBabich == true && babich != 0) {
						...
						else if (i == 1 && whoHaveBabich == true && babich != 0) {
							
							whoIsFristBeginRound = handleThreadList.get(1).accountName;
							// client đầu tiên trong danh sách sẽ là người có lượt đầu tiên và là người bắt đầu round đầu tiên
							responseDeckShuffled = new ResponseDeckShuffled(deckShuffled, nameOfAnotherPlayer, true, true);
							// đã tích hợp nội dung isFristBeginRound vào gói tin ở trên
							whoHaveBabich = false;
						}
						else if (i == 2 && whoHaveBabich == true && babich != 0) {
							
							whoIsFristBeginRound = handleThreadList.get(2).accountName;
							// client đầu tiên trong danh sách sẽ là người có lượt đầu tiên và là người bắt đầu round đầu tiên
							responseDeckShuffled = new ResponseDeckShuffled(deckShuffled, nameOfAnotherPlayer, true, true);
							// đã tích hợp nội dung isFristBeginRound vào gói tin ở trên
							whoHaveBabich = false;
						}
						else if (i == 3 && whoHaveBabich == true && babich != 0) {
							
							whoIsFristBeginRound = handleThreadList.get(3).accountName;
							// client đầu tiên trong danh sách sẽ là người có lượt đầu tiên và là người bắt đầu round đầu tiên
							responseDeckShuffled = new ResponseDeckShuffled(deckShuffled, nameOfAnotherPlayer, true, true);
							// đã tích hợp nội dung isFristBeginRound vào gói tin ở trên
							whoHaveBabich = false;
			...
			case "DeclareWinner":
				...
				whoDeclareWinner = declareWinner.getAccountName();
				
//				for (int i = 0; i < handleThreadList.size(); i++) {
//					if (whoDeclareWinner.equals(handleThreadList.get(i).accountName) && whoDeclareWinner == handleThreadList.get(i).accountName) {
//						
//						System.out.println(handleThreadList.get(i).accountName+"winnnneerrrr....");
//						ResponseWinner responseWinner = new ResponseWinner();
//						DataPacket dataPacket5 = (DataPacket) responseWinner;
//						handleThreadList.get(i).doWrite(dataPacket5);
//						
//					}
//					
//				}
				
				System.out.println(whoDeclareWinner+"winnnneerrrr....");
//				ResponseWinner responseWinner = new ResponseWinner();
//				DataPacket dataPacket5 = (DataPacket) responseWinner;
//				handleThreadList.get(0).doWrite(dataPacket5);
...
//				File file = new File("C:\\Users\\Acer\\Downloads\\" + requestSendFileData.getFile().getName());
				File file = new File("D:\\word\\" + requestSendFileData.getFile().getName());
